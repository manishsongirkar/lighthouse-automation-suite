    ...
    try:
		...
		<style>
			...
			@media (max-width: 768px) {
				.navigation-section { margin: 16px; padding: 16px; }
				.navigation-list { gap: 8px; }
				.navigation-item { padding: 10px 12px; }
		...
		<script>
			...
			document.addEventListener( 'DOMContentLoaded', function() {
				...
				if ( 'IntersectionObserver' in window ) {
					...
					const urlSections = document.querySelectorAll( '[style*="margin-bottom: 64px"]' );
					if ( urlSections ) {
			...
			function initializeNavigation() {
				...
				const sections = document.querySelectorAll( '[id^="url-"]' );
				...
				if ( window ) {
					window.addEventListener( 'scroll', function() {
						...
						scrollTimeout = setTimeout( handleScrollHighlight, 10 );
					} );
				}
...
<body>
	<div class="container">
        ...
        for index, (_, row) in enumerate(df.iterrows(), 1):
            ...
            navigation_html += f"""
				<li class="navigation-item">
					<a href="#{url_id}" class="navigation-link">
						<span class="navigation-icon">🌐</span>
        ...
        # Add the existing performance tables (mobile and desktop) with IDs
...
def generate_performance_tables(df):
    ...
    for index, (_, row) in enumerate(df.iterrows(), 1):
		...
		<div id="url-{index}" style="margin-bottom: 64px; border-bottom: 1px solid var(--psi-gray-200); padding-bottom: 32px; scroll-margin-top: 80px;">
